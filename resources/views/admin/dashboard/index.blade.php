<x-layouts.admin title="Dashboard Admin">
    ...
    <div class="admin-page-header">
        <div>
            ...
            <p class="admin-page-subtitle">Selamat datang kembali, <span class="font-medium text-slate-700">{{ auth('admin')->user()->name }}</span>!</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
        <div class="lg:col-span-2 dashboard-panel p-4 sm:p-5">
            <div class="flex items-center justify-between gap-3 mb-4">
                <h2 class="font-medium text-slate-800">Statistik Penyerapan Alumni</h2>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6">
                    <div class="w-40 h-40 shrink-0 relative">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-2xl font-bold text-slate-800">{{ $alumniTotalDashboard }}</span>
                            <span class="text-[11px] text-slate-400">Total Alumni</span>
                        </div>
                    </div>
                    ...
                    <div class="w-full space-y-3">
                        <div class="flex items-center justify-between rounded-lg bg-blue-50 px-4 py-3">
                            ...
                            <span class="text-sm font-semibold text-slate-800">
                                {{ $alumniBekerja }}
                                <span class="text-slate-400 font-normal">({{ round($alumniBekerja / $alumniTotalDashboard * 100) }}%)</span>
                            </span>
                        </div>
                        <div class="flex items-center justify-between rounded-lg bg-amber-50 px-4 py-3">
                            ...
                            <span class="text-sm font-semibold text-slate-800">
                                ...
                                <span class="text-slate-400 font-normal">({{ round($alumniMelanjutkanStudi / $alumniTotalDashboard * 100) }}%)</span>
                            </span>
                        </div>
                        <div class="flex items-center justify-between rounded-lg bg-slate-100 px-4 py-3">
                            <span class="flex items-center gap-2 text-sm text-slate-600">
                                <span class="w-3 h-3 rounded-full bg-slate-400"></span> Belum Bekerja
                            </span>
                            <span class="text-sm font-semibold text-slate-800">
                                {{ $alumniBelumBekerja }}
                                <span class="text-slate-400 font-normal">({{ round($alumniBelumBekerja / $alumniTotalDashboard * 100) }}%)</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                @if ($alumniPerTahun->isNotEmpty())
                    <div class="mt-5 border-t border-slate-100 pt-5">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-sm font-medium text-slate-700">Penyerapan Alumni per Tahun</h3>
                        </div>
                        <div class="h-64">
            ...
            @else
                <div class="flex flex-col items-center justify-center text-center py-10">
                    <svg class="w-12 h-12 text-slate-200 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4zm6 1a3 3 0 10-3-3 3 3 0 003 3z"/>
                    </svg>
                    <p class="text-sm text-slate-400">Belum ada data alumni untuk ditampilkan.</p>
                </div>
            @endif
        </div>

        <div class="dashboard-panel p-4 sm:p-5">
            <div class="flex items-center justify-between gap-3 mb-4">
                <h2 class="font-medium text-slate-800">Menunggu Persetujuan</h2>
            ...
            <div class="space-y-3">
                @forelse ($lowonganMenunggu as $item)
                    <div class="flex items-center justify-between text-sm border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                        <div class="min-w-0">
                            <p class="font-medium text-slate-800 truncate">{{ $item->posisi }}</p>
                            <p class="text-slate-400 text-xs truncate">{{ $item->mitra->nama_perusahaan }}</p>
                        </div>
                ...
                @empty
                    <div class="flex flex-col items-center justify-center text-center py-8">
                        <svg class="w-10 h-10 text-slate-200 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <p class="text-sm text-slate-400">Tidak ada lowongan menunggu.</p>
                    </div>
                @endforelse
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="dashboard-panel p-4 sm:p-5">
            <h2 class="font-medium text-slate-800 mb-4">Lowongan Berdasarkan Status</h2>
            ...
            @else
                <div class="flex flex-col items-center justify-center text-center py-10">
                    ...
                    <p class="text-sm text-slate-400">Belum ada data lowongan.</p>
                </div>
            @endif
        </div>

        <div class="dashboard-panel p-4 sm:p-5">
            <h2 class="font-medium text-slate-800 mb-4">Top Lowongan Dilihat</h2>

            @if ($topLowongan->isNotEmpty())
                <div class="space-y-3">
                    @foreach ($topLowongan as $i => $item)
                        <div class="flex items-center justify-between text-sm {{ $loop->last ? '' : 'border-b border-slate-100 pb-3' }}">
                            <div class="flex items-center gap-3 min-w-0">
                                <span class="w-6 h-6 flex items-center justify-center rounded-full {{ $i < 3 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500' }} text-xs font-medium shrink-0">{{ $i + 1 }}</span>
                                <div class="min-w-0">
                                    <p class="font-medium text-slate-800 truncate">{{ $item->posisi }}</p>
                                    <p class="text-slate-400 text-xs truncate">{{ $item->mitra->nama_perusahaan }}</p>
                                </div>
                            </div>
                            <span class="flex items-center gap-1 text-slate-500 text-xs shrink-0 ml-2">
            ...
            @else
                <div class="flex flex-col items-center justify-center text-center py-10">
                    ...
                    <p class="text-sm text-slate-400">Belum ada lowongan untuk ditampilkan.</p>
                </div>
            @endif
        </div>
    </div>
