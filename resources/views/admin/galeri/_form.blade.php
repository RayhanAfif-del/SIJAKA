...
<div class="mb-5">
    ...
    <input type="date" name="tanggal" value="{{ old('tanggal', isset($galeri) ? $galeri->tanggal->format('Y-m-d') : '') }}" required
        class="w-full sm:w-56 rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
...
<div class="mb-6">
    ...
    @if (!empty($galeri->foto))
        <img src="{{ Storage::url($galeri->foto) }}" class="w-28 h-20 rounded-lg object-cover mb-2">
    @endif
